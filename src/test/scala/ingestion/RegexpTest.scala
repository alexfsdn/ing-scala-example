...
class RegexpTest {
  ...
  def test(): Unit = {
    ...
    //hiveContext.sql("select cpf, regexp_replace(cpf, '[^0-9]', '') as CPF_TRATADO, telefone, regexp_replace(telefone, '[^0-9]', '') as TELEFONE_TRATADO from testDB.testTable").show(10, false)
    hiveContext.sql("select " +
      ...
      "  regexp_replace(substr(regexp_replace(telefone, '[^0-9]', ''), 3, length(telefone) - 2), '^0+', '')" +
      ") " +
      "ELSE " +
